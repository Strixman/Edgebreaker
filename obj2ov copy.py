...
def split_triangles_by_component(triangles):
    # Build a mapping from vertex to the list of triangle indices that use that vertex.
    vertex_to_tri = {}
    for tidx, tri in enumerate(triangles):
        for v in tri:
            vertex_to_tri.setdefault(v, []).append(tidx)
    ...
    for i in range(len(triangles)):
        ...
        while stack:
            ...
            visited[t] = True
            comp.append(t)
            # For each vertex in the triangle, check connected triangles.
            for v in triangles[t]:
                for neighbor in vertex_to_tri[v]:
...
def ov2obj(infile, outfile):
    ...
    cc = split_triangles_by_component(triangles)
    print(cc[0])
    ...
    return vertices, triangles, V, O, dummy_indices

if __name__ == "__main__":
    verticies, triangles, V, O, dummy_indices = ov2obj(sys.argv[1], sys.argv[2])
    print(dummy_indices)